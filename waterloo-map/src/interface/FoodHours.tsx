import { isCurrentFoodException, splitFoodMealHours } from "../utils/foodHoursDisplay";
...
export default function FoodHours({ hours, exceptions }: Pick<FoodInfo, "hours" | "exceptions">) {
  ...
  return <section className="mb-4" aria-label="Hours">
    ...
    <dl className="mt-2 space-y-2">
      {groups.map(group => <div key={group.first} className="text-ui-meta grid grid-cols-[4.5rem_minmax(0,1fr)] gap-3">
        <dt className="text-slate-500">{days[group.first].slice(0, 3)}{group.last !== group.first && ` – ${days[group.last].slice(0, 3)}`}</dt>
        <dd className="space-y-1.5 text-right font-medium text-slate-700">{splitFoodMealHours(group.hours).map((meal, index) => <div key={index}>{formatDisplayTime(meal)}</div>)}</dd>
      </div>)}
    </dl>
    {exceptions?.filter(note => isCurrentFoodException(note)).map(note => <p key={note} className="text-ui-meta mt-2 leading-relaxed text-slate-500">{formatDisplayTime(note)}</p>)}
  </section>;
}

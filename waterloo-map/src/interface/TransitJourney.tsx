...
export default function TransitJourney({ origin, destination, map, walkingMinutes, onRouteReady, onLegendChange }: { onLegendChange?: (entries: LegendEntry[]) => void; onRouteReady?: (coordinates: Point[]) => void; walkingMinutes?: number; origin: Point; destination: Point; map: mapboxgl.Map }) {
  ...
  useEffect(() => {
    const color = selected ? transitRouteColor(selected.mode === "ion" ? "ion" : "bus", selected.route) : "#64748b";
    onLegendChange?.(selected ? [
      { id: "trip-walk", label: "Walk to / from transit", symbol: "walk", color: "#13735a" },
      { id: "trip-ride", label: `${selected.mode === "ion" ? "ION" : "Bus"} ${selected.route} path`, symbol: "line", color },
      { id: "trip-stops", label: "Stops on your trip", symbol: selected.mode === "ion" ? "station" : "circle", color },
    ] : []);
    return () => onLegendChange?.([]);
  }, [selected, onLegendChange]);

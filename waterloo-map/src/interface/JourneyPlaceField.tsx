...
export default function JourneyPlaceField({ label, value, places, onSelect, onMap, onEdit }: { label: string; value?: string; places: JourneyPlace[]; onSelect: (place: JourneyPlace) => void; onMap: () => void; onEdit?: () => void }) {
  ...
  return <div className="rounded-xl border border-emerald-100 bg-emerald-50/50 p-3">
    <label className="flex items-center gap-2 text-sm"><span className="w-9 shrink-0 font-medium text-[#13735a]">{label}</span><Search size={15} className="shrink-0 text-slate-400" /><input aria-label={`${label} location`} value={query ?? value ?? ""} onFocus={()=>{onEdit?.();setQuery("");}} onChange={event=>setQuery(event.target.value)} onKeyDown={event=>{if(event.key==='Escape')setQuery(null);if(event.key==='Enter'&&matches[0]){onSelect(matches[0]);setQuery(null);}}} placeholder="Search buildings or stops" className="min-w-0 flex-1 bg-transparent text-slate-900 outline-none" /></label>

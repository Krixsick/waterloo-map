...
export default function EventsPanel({events, filter, venue, loading, error, onFilter, onClearVenue, onSelect, onClose, onRetry}: {
  ...
  return <section aria-label="Event map filters" className="map-panel pointer-events-none">
    <div className="pointer-events-auto flex w-fit max-w-full flex-wrap items-center rounded-3xl border border-slate-200 bg-white/95 p-1 shadow-sm backdrop-blur sm:gap-1">
      {([['today','Today'],['week','This week'],['month','This month']] as const).map(([value,label]) => <button type="button" key={value} aria-pressed={filter===value} onClick={()=>onFilter(value)} className={`min-h-11 cursor-pointer whitespace-nowrap rounded-full px-2 py-2 text-sm transition-colors focus-visible:outline-2 focus-visible:outline-violet-600 sm:px-3 ${filter===value?'bg-violet-50 font-medium text-[#7c3aed]':'text-slate-600 hover:bg-slate-50'}`}>{label}</button>)}
      <button type="button" aria-label="Close events" onClick={onClose} className="flex size-11 shrink-0 items-center justify-center rounded-full text-slate-500 hover:bg-slate-100"><X size={16}/></button>
    </div>
